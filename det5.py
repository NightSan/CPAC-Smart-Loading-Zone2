...
def save_snapshot(frame, label: str, snapshot_dir: str) -> str:
    """Save image to disk, return file path"""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_label = label.replace(" ", "_")
    filename = f"{safe_label}_{ts}.jpg"
    path = os.path.join(snapshot_dir, filename)
    cv2.imwrite(path, frame)
    print(f"[SNAPSHOT] saved → {path}")
    return path

def send_api_event(frame, event_type: str, image_path: Optional[str] = None):
    """
    Send event to Camera Events API (runs in background thread)

    Supported event_type values:
      • have_truck_waiting  → truck enters zone (ARRIVED)
      • truck_loading       → truck parked and loading (LOADING) — attaches saved image
      • truck_loading       → loading done / truck leaving (DONE) — attaches current frame
    """
    def _post():
        try:
            ...
            if image_path and os.path.exists(image_path):
                # LOADING: use the saved file
                with open(image_path, "rb") as f:
                    img_bytes = f.read()
                files = {"image": ("snapshot.jpg", img_bytes, "image/jpeg")}
            else:
                # ARRIVED / DONE: encode current frame instead
                ok, buf = cv2.imencode(".jpg", frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
                if not ok:
                    print(f"[API] failed to encode frame")
                    return
                files = {"image": ("snapshot.jpg", buf.tobytes(), "image/jpeg")}

            resp = requests.post(API_URL, headers=headers, data=data, files=files, timeout=10)
            print(f"[API] {event_type} → {resp.status_code} {resp.json().get('message','')}")
...
SNAPSHOT_DIR  = r"C:\Users\HP\OneDrive\Desktop\Yolotest3\snapshots"

os.makedirs(SNAPSHOT_DIR, exist_ok=True)

# --- API ---
API_URL       = "https://cpaciot.cipcloud.net/iot-service/api/v1/camera-events"
API_KEY       = "camera-event-secret-token-2026"
...
def on_mouse(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        print(f"[CALIBRATE] Clicked at x={x}, y={y}  →  set DROP_POINT_X={x}, DROP_POINT_Y={y}")
...
if __name__ == '__main__':
    ...
    print(f"[SNAPSHOT] Saving images to → {SNAPSHOT_DIR}")
    ...
    while True:
        ...
        # ==========================================
        # 2.5 Snapshot + API on State Change
        # ==========================================
        if current_state != prev_state:
            snap_label = locked_truck_type or "UNKNOWN"

            if current_state == "LOADING":
                # Save image + send API: truck_loading with attached image
                img_path = save_snapshot(frame, f"LOADING_{snap_label}", SNAPSHOT_DIR)
                print(f"[EVENT] LOADING_{snap_label} → truck_loading + image")
                send_api_event(frame, "truck_loading", image_path=img_path)
